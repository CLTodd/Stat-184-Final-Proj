...
In search of n answer, we'll approach this question in 3 ways:  
...
**3. Are Hate Crime rates growing? And, if so, are they growing at a faster rate than the general Crime Rate?**  
...
### Analysis
Recall our 3 approaches to the research question:  
...
**Are Hate Crimes becoming a larger proportion of all crimes committed?** To answer this, we need to wrangle our data to express the proportiong of crimes that were Hate Crime rates for each of the states for every year. To make a legible plot, we'll use national yearly summaries instead of grouping data by state.
...
HateProportions %>%
  ...
  xlab("Year")
```
The proportion of each year's crimes that were hate crimes is very small, but this was expected as Hate crimes are a very specific and generally infrequent kind of crime. Once again, we see a general decrease in hate crimes from 2008 to 2014 and an increase from 2008 to 2014. This plot seems to imply that Hate Crimes started becoming a larger proportion of all crimes committed in 2014. Now we can see that the post-2014 increase is much more consistent than the pre-2014 decrease. From 2014 to 2018, the proportion of crimes that were hate crimes increased by about 50%. We'll apply another linear model, separating at 2014 again.

```{r}
HP1 <- lm(propHate ~ year, data = filter(HateProportions, year < 2014))
HP2 <- lm(propHate ~ year, data = filter(HateProportions, year >= 2014))

#Once again, the coefficients of both linear models are very small. However, this time we might be able to see some significance."
...
a significance level of 0.01 indicates that there is less than a 0.1% chance that the coefficient might be equal to 0 and thus be insignificant.


```{r}
#Hate crime growth rates
HateGrowthRate <-
