...
Crime2008 <-
Crime2008 %>%
  transmute(state = tolower(`Table.5`), # making all lowercase for consistency's sake, because some tables have states in all caps
            total = as.numeric(X2), #        In the raw data, the `X2` column actually represents the area of the state where the crim occured,
...
Hate2008 <-
Hate2008 %>%
  transmute(state = tolower(`Table.13`),
...
#Renaming the useful columns, re-classing variabels, and dropping (ignoring) columns that aren't of interest to the research question
Crime2009 <- Crime2009 %>%
  transmute(state = tolower(gsub("(\\d*\\,*\\d+)$", "", `Table.5`)), # Some state names have numbers on the end that need to be trimmed, 
  total = as.numeric(X2), 
